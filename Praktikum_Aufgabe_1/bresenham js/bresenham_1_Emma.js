...
/**
 * Zeichnet eine Linie von (x0, y0) nach (x1,y1) im ersten Oktanten.
...
function drawLine(x0, y0, x1, y1){
  ...
  let a = -deltaY; //hier der Wert negiert, da y-Wert nach oben hin negativ wird (y-Achse zeigt nach unten)
  let b = -deltaX; 
  
  let q = 2 * a + b; //qinit --> erster Abstand zwischen M(x+1,y+1/2) und der Geraden
  let q_step = 2 * (a + b); //q_step, Abstand, der addiert wird, wenn Gerade über M
  let q_equal = 2 * a; //q_equal, Abstand, der addiert wird, wenn Gerade unter M
  ...
  for (let x = x0; x <= x1; x++){ //x wird immer um 1 erhöht, bis es den Endpunkt erreicht
    ...
    if (q < 0){         //Linie liegt unterhalb des Mittelpunkts
      q = q + q_equal;  //Abstand mit q_equal aktualisieren
                        //bleiben in der gleichen Zeile
    } else{             //Linie liegt oberhalb oder auf dem Mittelpunkt
      q = q + q_step;   //Abstand mit q_step aktualisieren
      y--;              //eine Zeile nach oben
      //hier y dekrementieren, da y-Achse nach unten zeigt... der y-Wert muss also sinken, wenn das Pixel optisch weiter oben gezeichnet werden soll
    }
  }
